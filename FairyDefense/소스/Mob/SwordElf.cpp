...
void SwordElf::Init(Layer* pLayerMainScene)
{
	...
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Mob/SwordElf/SwordElfAnim.plist");

	m_pSprCharacter = GameManager::GetInstance()->GetSpriteWithFrameFileName("SwordElf_00.png");
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (i == 0)
		{
			for (int i = 0; i <= RES_PUPPET_02; i++)
			{
				...
				sprintf(szFileName, "SwordElf_%.2d.png", i);
				frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(szFileName);
				vecFrame.pushBack(frame);
			}
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("SwordElf_01.png");
			vecFrame.pushBack(frame);
		...
		else
		{
			...
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("SwordElf_04.png");
			vecFrame.pushBack(frame);
...
void SwordElf::Update(float fElapse)
{
	if (m_bAlive == false)
	{
