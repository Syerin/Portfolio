...
void WizardElf::Init(Layer* pLayerMainScene)
{
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Mob/WizardElf/WizardElfAnim.plist");

	Size visibleSize = Director::getInstance()->getVisibleSize();

	m_pLayerMainScene = pLayerMainScene;

	m_pSprCharacter = GameManager::GetInstance()->GetSpriteWithFrameFileName("WizardElf_00.png");
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (i == 0)
		{
			for (int i = 0; i <= RES_PUPPET_02; i++)
			{
				...
				sprintf(szFileName, "WizardElf_%.2d.png", i);
				frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(szFileName);
				vecFrame.pushBack(frame);
			}
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("WizardElf_01.png");
			vecFrame.pushBack(frame);
		...
		else
		{
			...
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("WizardElf_04.png");
			vecFrame.pushBack(frame);
...
void WizardElf::Update(float fElapse)
{
	if (m_bAlive == false)
	{
