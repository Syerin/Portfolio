...
void Rabbit::Init(Layer* pLayerMainScene)
{
	m_pLayerMainScene = pLayerMainScene;
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Mob/Rabit/RabitAnim.plist");
	...
	m_vec2Pos.y = 200;
	m_bAlive = false;
	m_eMobType = MOB_TYPE_RABIT;

	m_pSprCharacter = GameManager::GetInstance()->GetSpriteWithFrameFileName("Rabit_00.png");
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (i == 0)
		{
			for (int i = 0; i <= RES_PUPPET_02; i++)
			{
				...
				sprintf(szFileName, "Rabit_%.2d.png", i);
				frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(szFileName);
				vecFrame.pushBack(frame);
			}
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("Rabit_01.png");
			vecFrame.pushBack(frame);
		...
		else
		{
			...
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName("Rabit_04.png");
			vecFrame.pushBack(frame);
...
void Rabbit::Update(float fElapse)
{
	if (m_bAlive == false)
	{
